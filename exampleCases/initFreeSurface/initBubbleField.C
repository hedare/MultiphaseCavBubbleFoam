...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    dimensionedScalar sheetThickness("sheetThickness", dimLength, transportProperties);
    
    dimensionedScalar epsDtB_air("epsDtB_air", dimLength*dimLength, transportProperties);
    
    dimensionedScalar epsDtB_oil("epsDtB_oil", dimLength*dimLength, transportProperties);
    ...
    dimensionedScalar rho = rho0_water*pow((pliquid+B_water)/(p0_water+B_water),1/gamma_water);
    ...
    forAll(alpha_water, cellI)
    {
        ...
        {
            alpha_oil[cellI] = 1 - alpha_air[cellI];
            alpha_air[cellI] = 0;
        }
...
//smear the interface to avoid the numerical instability grows in the beginning
    ...
    if (epsDtB_oil.value() != 0)
    {
        volScalarField alpha_oil0(alpha_oil);
        fvScalarMatrix alpha_oilSEqn
        (
            fvm::Sp(scalar(1),alpha_oil) - fvm::laplacian(epsDtB_oil,alpha_oil) == alpha_oil0
        );
        alpha_oilSEqn.solve();
    }

    alpha_water = max(min(1.0-alpha_air-alpha_oil,1.0),0.0);
