...
void Foam::multiphaseMixtureThermo::correctComponent(phaseModel& phase)
{
    phase.correct();
}

void Foam::multiphaseMixtureThermo::correctFinal()
{
    auto phasei = phases_.cbegin();
...
void Foam::multiphaseMixtureThermo::solve()//const scalarField& p)
{
...
{
    scalar limitAlpha = mesh_.solutionDict().subDict("mysolver").lookupOrDefault<scalar>("limitAlpha", 1.0); //scalar limitAlpha = 0.99;
    //bool cavModel = mesh_.solutionDict().subDict("mysolver").lookupOrDefault<Switch>("cavModel", false);
    
    if (limitAlpha <= 0.0 || limitAlpha > 1) { limitAlpha = 1.0; }
    ...
    for (phaseModel& alpha : phases_)
    {
...

        {
            ...
            forAll(dgdt, celli)
            {
                if (dgdt[celli] < 0.0 && alpha[celli] > (1 - limitAlpha))
                {
                    Sp[celli] += dgdt[celli]*alpha[celli];
                    Su[celli] -= dgdt[celli]*alpha[celli];
                }
                else if (dgdt[celli] > 0.0 && alpha[celli] < limitAlpha)
                {
        ...
        for (const phaseModel& alpha2 : phases_)
        {
            ...
            forAll(dgdt2, celli)
            {
                if (dgdt2[celli] > 0.0 && alpha2[celli] < limitAlpha)
                {
                    Sp[celli] -= dgdt2[celli]*(1.0 - alpha2[celli]);
                    Su[celli] += dgdt2[celli]*alpha[celli];
                }
                else if (dgdt2[celli] < 0.0 && alpha2[celli] > (1 - limitAlpha))
                {
                    Sp[celli] += dgdt2[celli]*alpha2[celli];
                }
            }
            
            /*if (cavModel && alpha.name() == "water" && alpha2.name() == "oil")
            {
                ...
                dimensionedScalar pSat = transportProperties_.lookup("pSat"));
                dimensionedScalar nSeed = transportProperties_.lookup("nSeed"));
                dimensionedScalar dNuc = transportProperties_.lookup("dNuc"));
                scalar alpha0 = 1.0/(1.0/(nSeed.value()*4/3*3.14159265358979*pow(dNuc.value(),3))+1.0);
                dimensionedScalar cavSourceUnit("cavSourceUnit",dimPressure*dimTime, 1.0);
                volScalarField cavSource
                (
                    IOobject
                    (
                        "cavSource",
                    ...
                    (p-pSat)*alpha2.thermo().rho()/rho()*pow(alpha.thermo().rho()*pos(p-pSat),-1/2)*pow((1-alpha)*pow(alpha,2),1/3)*pow(6,1/2)*pow(4*3.14159265358979*nSeed/3,1/3)/cavSourceUnit
                );
	        Su += cavSource;
	        Sp -= cavSource;
	    }*/
